...
for i = 1:num_images
    ...
    imgf = flip(img,2); %for data augm
    hog = vl_hog(img, feature_params.hog_cell_size); %HOG
    hogf = vl_hog(imgf, feature_params.hog_cell_size); %HOG
    features_pos(2*i-1,:) = reshape(hog, 1, D); %Reshape (as vector)
    features_pos(2*i,:) = reshape(hogf, 1, D); %Reshape (as vector)
end
